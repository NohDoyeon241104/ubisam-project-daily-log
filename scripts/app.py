...
REPO_ROOT = SCRIPT_DIR.parent            # scripts/ 의 상위 = 레포 루트
PROJECTS_JSON = REPO_ROOT / "projects.json"
OUTPUT_DIR = REPO_ROOT / "output"        # 일일 로그 생성 위치 (git 제외)
UPLOADS_DIR = REPO_ROOT / "uploads"      # 업로드 파일 위치 (git 제외)


def resolve_root(log_root: str) -> Path:
    """log_root 가 상대경로면 레포 루트 기준으로 해석."""
    p = Path(log_root)
    return p if p.is_absolute() else (REPO_ROOT / log_root)
